...
(function() {
  ...
  function pmxdr(host, onload) {
    ...
    instance.iFrame.onload = function() {
      if (typeof instance.onload == "function") instance.onload();
    };
  ...
  function pmxdrResponseHandler(evt) {
    ...
    if (data.pmxdr == true) { // only handle pmxdr requests
      if (
        ...
        if (pmxdr.requests[data.id]) delete pmxdr.requests[data.id];
      }
    }
  }
  ...
  pmxdr.destruct = function() {
    if (window.removeEventListener) window.removeEventListener("message", pmxdrResponseHandler, false);
    else if (window.detachEvent) window.detachEvent("onmessage", pmxdrResponseHandler);
    delete window.pmxdr;
