  ...
  function handlePmxdrReq(evt) {
    ...
    if ( data.pmxdr == true ) { // only handle pmxdr requests
      ...
      for (var i=0; i<alwaysTrustedOrigins.length; i++) {
        if (alwaysTrustedOrigins[i] instanceof RegExp) alwaysTrusted = alwaysTrustedOrigins[i].test(evt.origin);
        else if (typeof alwaysTrustedOrigins[i] == "string") alwaysTrusted = (origin === alwaysTrustedOrigins[i]);
      }
          ...
          if(req.status == 200) {
                 ...
                 || ac.origin.indexOf(origin) != -1 )
            {
                  ...
                  || (typeof ac.methods == "string" && ac.methods.indexOf(data.method) != -1) )
              {
                ...
                var responseHeaders = req.getAllResponseHeaders().split(/\r?\n/);
               ...
               // } else return err("DISALLOWED_HEADERS"); // At least one header was not allowed - nvm, that part of the spec can't
