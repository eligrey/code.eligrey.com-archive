/*  License
Noteboard - v2.0.1 - noteboard.eligrey.com
Creative Commons (CC) License:
Attribution-Noncommercial-Share Alike 3.0 Unported
http://creativecommons.org/licenses/by-nc-sa/3.0/
Author: Elijah Grey - www.eligrey.com
const Contact.Email.Address = ['\u0065\u006c\u0069\u006a\u0061\u0068','\u0067\u0072\u0065\u0079\u002e\u006e\u0061\u006d\u0065'].join('\u0040');
*/

/*	Not going to implement support for Google Gears, Flash storage, or any other third-party (proprietary or otherwise) storage interface.
		1. Less work for me.
		2. Forces you to upgrade your IE to IE8 if you're using IE.
		3. noteboard.js is smaller
		4. standards ftw! :P
*/	

//nbTitle should be declared before packed script
...
//{ init vars
var host = location.hostname || (window.opener ? window.opener.location.hostname : false) || undefined,
nbArea,
statsBar,
tab_code = '\t',
isCtrl = false,
...
if ( typeof(window.crypto.alert) != 'undefined' )
	...
	alert = function(msg){ window.crypto.alert('Noteboard at ' + host + (msg?' says:\n\n'+msg:'')) };
	}
*/
...
err = function(err_msg, lineN, uriOverride)
{
...
if ( typeof(globalStorage) != 'undefined' && typeof(localStorage) == 'undefined' && host !== 'unefined' )
	{
...
function autoLoadDoc()
{
	...
	{
		isAutoloadURL = true;
		loadDoc(decodeURIComponent(location.hash.substr(5)));
	}
};
...
'Bookmark Noteboard \u2014 Ctrl + D (Title will auto-change for bookmarking non-autoload URLs)',
...
function init()
{
	...
	*/
	if ( localStorage.getItem('nb') !== null && localStorage.getItem('nb') !== undefined )
	{
		...
		if ( nb.previous_doc == '' ) { nb.previous_doc = 'shortcuts'; }
...
if ( e.keyCode == 85 && isAlt == true && ( isCtrl == true || isShift == true ) )  // Alt + Shift + U and Ctrl + Alt + U; Get Document Autoload URLs,
	{
		...
		if ( location.hash != '' && location.search == '' ) { autoLoadURL = autoLoadURL.split(location.href.search(/#/))[0]; }
		...
		/*old, delete after ^ checked to work
...
if ( e.keyCode == 68 && isAlt == true && isCtrl == true && isShift == false ) // Alt + Ctrl + D; delete all data  w/ confirmation
	{
		var reallyDelete = confirm('You are about to delete all data stored by Noteboard on '+host);
		if ( reallyDelete == true )
		{
			deleteAllData();
			alert('All Noteboard data on '+host+' deleted.');
		}
		else { alert('No documents deleted.'); }
		stopEvent(e);
		updateStatsBar();
...
function nbJSON()
{
	//return ({'version': version, 'docs': nb.docs, 'previous_doc': nb.previous_doc, 'statsBarOn': nb.statsBarOn});
...
function saveNB()
{
	...
	//	localStorage.setItem('nb', ({docs: nb.docs, previous_doc:nb.previous_doc}));
};
